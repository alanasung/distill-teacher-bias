# Design Document: When Distillation Makes Monitors Prefer Their Teacher

## 1. Purpose

This document is the method contract for **When Distillation Makes Monitors Prefer Their Teacher**. It specifies what is
...
One-liner: Test whether distilled monitors systematically favor their teacher model's attributions under self-monitoring.

Hypothesis: Test whether distilled monitors systematically favor their teacher model's attributions under self-monitoring.
...
|---|---|---|
| Config schema | `distbias.configs.schema` | Typed dataclasses + role-keyed models |
| Loader | `distbias.configs.loader` | Hydra compose API |
| Device | `distbias.models.device` | MPS/CUDA/CPU resolution + fallback flag |
| Registry | `distbias.models.registry` | Pinned revisions + hardware fit |
| Hooks | `distbias.models.hooks` | Capture, steer, ablate, patch, attn intervene |
| Generation | `distbias.models.generation` | Chat template path recording |
| Data | `distbias.data` | Synthetic items, splits, manifests |
| Evaluation | `distbias.evaluation` | Metrics, failure harness, protocols |
| Ablation | `distbias.ablation` | Control / dropout / seed-sweep arms |
| Reporting | `distbias.reporting` | Aggregate → MD/TeX/figures |
| Cache | `distbias.cache` | Versioned artifact store |
...
Mentor(s): Alana Sung
Affiliation(s): 
SPAR posting: https://github.com/alanasung/distill-teacher-bias
...
`src/distbias/distbias/` are preserved across regeneration.
